# Generates Hawkes time series for test set
...
def generate_hawkes1():
    [T] = simulate_hawkes(400,0.2,[0.6, 0.4],[1.0, 20.0])
    #score = - LL[32:].mean()
    return [T]
    
def generate_hawkes2():
    [T] = simulate_hawkes(400,0.3,[0.3, 0.5],[1.0, 20.0])
    #score = - LL[32:].mean()
    return [T]
...
def generate_hawkes3():
    [Tx] = generate_hawkes1()
    [Tz] = generate_hawkes2()
    [T] = simulate_hawkes_y(400,0.2,[0.6, 0.4],[1, 20], Tx, Tz)
    #score = - LL[80:].mean()
    return [T]
...
with open('time_alpha_test_1.txt', 'w') as output:
...
with open('time_beta_test_1.txt', 'w') as output:
...
with open('time_gamma_test_1.txt', 'w') as output:
